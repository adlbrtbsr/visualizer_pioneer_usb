...
class FractalState:
	...
	# Phase-interference instantaneous values
	phase_hue: float = 0.0
...
class FractalRenderer:
	...
	def _fs_src() -> str:
		return (
			...
			uniform vec2  u_phase_vec;
			uniform float u_phase_jitter;
			...
			void main(){
				...
				// Small per-pixel offset from phase interference
				vec2 phase_off = u_phase_vec * (0.5 + 0.5 * u_energy);
				float jitter = (hash12(gl_FragCoord.xy + 13.7) - 0.5) * (2.0/255.0) * u_phase_jitter;
				vec2 p = R * (Sh * ((p0 + phase_off) * u_pre_scale));
